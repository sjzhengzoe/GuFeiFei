...
router.get("/api/get/heatmap", async (ctx, next) => {
  ...
  // 参数
  const { dateStart, dateEnd, device = "" } = query;
  // 请求ga接口获取参数
  try {
    let responseData = await getChartData({ dateStart: dateStart ? new Date(dateStart) : new Date(), dateEnd: dateEnd ? new Date(dateEnd) : new Date(), device: device });
